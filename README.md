...
- **地图总览** — 二维地图上看全部文物点位：整市视角按保护级别着色圆点（国保红 / 省保橙 / 市保蓝 / 县保绿 / 未定级紫），放大后自动切换成"级别色圆底 + 类别剪影"的图标徽章（古建/墓葬/碑刻/近现代/遗址五种图形），左下角常驻图例，级别越高名字出现得越早。市域整域取景、市界发光描边、域外自动压暗（亮白主题下换浅色雾）。保护范围（红线）与建设控制地带（蓝线）常驻叠加、随缩放自动隐现，「边界」菜单可整层开关。底图默认天地图影像（在线影像/矢量 × 天地图/高德，另有离线底图）。点开任意一处看详情档案、照片图纸，有三维模型和普查档案的直接打开。筛选支持关键字、类别、区县、级别、年代、保存状况多维组合，统计图表点击即筛地图，顶部「当前视图」面包屑实时显示筛选层级（区域›级别›类别›年代›状况）与数量。
...
| 市域高亮边界 | `boundary/{市名}_市.geojson` 等 | 仓库自带种子,清除数据后自动恢复市界/县界底图 |
