...
面向"数据要素×"大赛文物保护利用赛道的市级平台：以"市级基础层 + 嘉祥县全量层"两级数据组织全市在级文物保护单位，围绕**数据要素门面、数据资源目录与开放共享、基层文物巡查、AI 文物问答**四大场景构建。

## 平台功能

| 模块 | 入口 | 说明 |
| --- | --- | --- |
| 地图总览 | `/app/#/` | Cesium 三维地图,点位/两线范围/筛选/详情档案 |
| 数据要素门面 | `/app/#/dashboard` | 资源总量、分级/县区/年代/保存状况分布、数据质量与巡查动态 |
...
- **市级基础层**（约 900 处在级保护单位,国/省/市/县四级）：名称、级别、类别、年代、坐标、两线范围、照片、图纸、简介。
- **嘉祥县全量层**（568 处不可移动文物）：在基础层要素之上，增加三普/四普档案查看、实景三维模型（约 400 个）。

### 巡查闭环

1. PC 端按保存状况分级确定巡查频率（差=每月、较差=双月、一般=季度……）,到期自动提醒。
2. AI 解析自然语言意图（"巡查武氏祠附近 5 处文物"）或手动点选/框选生成路线,接入高德驾车路线规划。
3. 路线生成二维码,手机扫码进入 H5:一键唤起高德导航（含途经点）、拍照打卡。
4. 照片 EXIF 定位与文物坐标自动比对核验"是否到场";PC 端 AI 视觉模型比对历史照片评估保存状况,一键生成巡查报告。
...
Windows 下可直接用 `start-backend.bat` / `start-frontend.bat`。
...
未配置 AI key 时问答与评估自动降级为规则模式；未配置高德 key 时路线用直线连接,均不影响启动。
